  ...
  worker_t worker(context, upstream_proxy_endpoint, downstream_proxy_endpoint, server_result_loopback, server_request_interrupt,
    [](const std::list<zmq::message_t>& messages, void* request_info, worker_t::interrupt_function_t&){
      ...
      try {
        ...
        //each time. this would require a worker being able to both respond and forward though..
        //TODO: use const unordered map of extension to mime-type and set a decent header
        size_t pos = 0;
        while(pos < request.path.size() && (request.path[pos] == '/' || request.path[pos] == '.'))
          ++pos;
