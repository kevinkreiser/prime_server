...
int main(int argc, char** argv) {
  ...
  const char* in_parse_lb = "ipc://in_parse_lb";
  const char* out_parse_lb = "ipc://out_parse_lb";
  const char* in_prime_lb = "ipc://in_prime_lb";
  const char* out_prime_lb = "ipc://out_prime_lb";
  const char* in_collector = "ipc://in_collector";

  //request producer
  size_t produced_requests = 0;
  producer request_producer(context_ptr, in_parse_lb,
  ...
  std::thread parse_balancer(std::bind(&load_balancer::balance, load_balancer(context_ptr, in_parse_lb, out_parse_lb)));
  parse_balancer.detach();

  //request parsers
  std::list<std::thread> parse_worker_threads;
  for(size_t i = 0; i < worker_concurrency; ++i) {
    parse_worker_threads.emplace_back(std::bind(&worker::work, worker(context_ptr, out_parse_lb, in_prime_lb,
  ...
  std::thread prime_balancer(std::bind(&load_balancer::balance, load_balancer(context_ptr, in_prime_lb, out_prime_lb)));
  prime_balancer.detach();

  //prime computers
  std::list<std::thread> prime_worker_threads;
  for(size_t i = 0; i < worker_concurrency; ++i) {
    prime_worker_threads.emplace_back(std::bind(&worker::work, worker(context_ptr, out_prime_lb, in_collector,
      [] (const std::list<zmq::message_t>& job) {
        //check if its prime
  ...
  std::thread collector_thread(std::bind(&collector::collect, collector(context_ptr, in_collector,
